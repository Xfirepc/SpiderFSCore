<?php

define('FS_COOKIES_EXPIRE', 604800);
define('FS_ROUTE', '');
define('FS_DB_FOREIGN_KEYS', true);
define('FS_DB_TYPE_CHECK', true);
//define('FS_MYSQL_CHARSET', 'utf8mb4');
//define('FS_MYSQL_COLLATE', 'utf8mb4_general_ci');
define('FS_MYSQL_CHARSET', 'utf8mb3');
define('FS_MYSQL_COLLATE', 'utf8mb3_bin');
define('FS_LANG', 'es_EC');
define('FS_TIMEZONE', 'America/Guayaquil');
define('FS_DB_TYPE', 'mysql');
define('FS_DB_HOST', 'mysql');
define('FS_DB_PORT', '3306');
define('FS_DB_NAME', 'spidercrm');
define('FS_DB_USER', 'root');
define('FS_DB_PASS', 'root');
define('FS_CACHE_HOST', '');
define('FS_CACHE_PORT', '');
define('FS_CACHE_PREFIX', '5p1d3r');
define('FS_HIDDEN_PLUGINS', '');
define('FS_DEBUG', true);
define('FS_DISABLE_ADD_PLUGINS', false);
define('FS_DISABLE_RM_PLUGINS', false);
